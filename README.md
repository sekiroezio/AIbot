...
基于 Spring AI + Spring AI Alibaba 构建的多模态智能个人助手系统，包含 **GraphRAG 知识图谱阅读助手** 和 **个人办公助手** 两大模块。
...
- **Todo 规划**：通过 Todo 工具进行任务分解与规划，展示结构化执行流程
- **记忆管理**：自定义动态 Token 比例窗口摘要 Hook，智能压缩对话历史，避免反复触发压缩
- **技能系统 (Skills)**：渐进式披露，轻松扩展技能
- **Shell 工具**：支持命令行操作，可执行脚本与系统命令
- 通过 MCP 协议集成外部工具服务 （如Tavily 网络搜索获取外部信息）
- 基于 Redis 的 Agent 记忆持久化
- 支持流式输出，展示思维过程和工具调用过程
...
│   │   ├── CommonConfiguration.java  # 核心Bean配置（Agent、Skills、Hooks等）
...
│   │   ├── PersonalAgentController.java  # 个人办公助手接口
│   │   ├── GraphController.java
│   │   └── PdfController.java
│   ├── advisor/                   # AI Advisor
│   │   └── GraphRagAdvisor.java   # 图谱增强Advisor
│   ├── hook/                      # Agent Hooks
│   │   └── CustomSummarizationHook.java  # 动态Token比例窗口摘要Hook
│   ├── service/                   # 业务逻辑层
│   │   ├── PersonalAgent.java     # 个人办公助手服务
│   │   ├── GraphService.java
│   │   ├── PdfService.java
...
│   └── static/                    # 前端页面
│       ├── index.html
│       ├── personalAgent.html     # 个人办公助手页面
│       └── graphrag.html
├── docs/                          # 技术方案文档
│   ├── skill-integration-plan.md  # Skills集成改造方案
│   └── memory-management-plan.md  # 记忆管理改进方案
...
技能包放置在 `src/main/resources/skills/` 目录下，Agent 启动时通过 `ClasspathSkillRegistry` 自动加载。每个技能目录需包含 `SKILL.md`（技能描述与提示词）和 `_meta.json`（元数据）。

### 7. 记忆管理配置

个人办公助手使用 `CustomSummarizationHook` 进行智能记忆压缩：

- **动态 Token 比例窗口**：不固定保留消息条数，而是根据 token 比例动态计算保留窗口
- **防死循环**：保留消息的 token 预算为阈值的 25%（`safeRatio=0.25`），保证压缩后不会反复触发
- **压缩日志**：记录压缩前后 token 数、减少量和压缩比例
...
| `/ai/service/stream` | POST | 流式问答（SSE） |
...
**Q: 发送长文本报错 "Request header is too large"？**
A: 个人办公助手接口已改为 POST + Request Body 传输，请确保浏览器已刷新加载最新页面（Ctrl+Shift+R）。

**Q: 记忆压缩频繁触发？**
A: `CustomSummarizationHook` 采用动态 Token 比例窗口，保留消息的 token 预算为阈值的 25%，理论上不会反复触发。可在 `CommonConfiguration.java` 中调整 `maxTokensBeforeSummary` 和 `safeRatio`。
