...
public class LLMEntityExtractor {
    ...
    private ExtractionResult deduplicate(ExtractionResult result) {
        ...
        List<Relationship> relationships = new ArrayList<>(result.getRelationships());

        Map<String, String> nameMapping = new HashMap<>();

        Set<String> seenNames = new HashSet<>();
        List<Entity> dedupedEntities = new ArrayList<>();

        for (Entity entity : entities) {
            String normalizedName = entity.getName().toLowerCase().trim();
            if (normalizedName.isEmpty()) continue;

            String existingCanonical = findSimilarEntity(normalizedName, dedupedEntities);
            if (existingCanonical != null) {
                nameMapping.put(entity.getName().trim(), existingCanonical);
                Entity existing = dedupedEntities.stream()
                        .filter(e -> e.getName().trim().equals(existingCanonical))
                        .findFirst().orElse(null);
                if (existing != null && (existing.getDescription() == null || existing.getDescription().isBlank())
        ...
        for (Map.Entry<String, String> entry : nameMapping.entrySet()) {
            log.debug("实体名称映射: {} -> {}", entry.getKey(), entry.getValue());
        }

        for (Relationship rel : relationships) {
            String source = rel.getSource().trim();
            String target = rel.getTarget().trim();
            rel.setSource(nameMapping.getOrDefault(source, source));
            rel.setTarget(nameMapping.getOrDefault(target, target));
        }

        Set<String> validNames = dedupedEntities.stream()
                .map(e -> e.getName().toLowerCase().trim())
                .collect(Collectors.toSet());

        relationships = relationships.stream()
                .filter(r -> validNames.contains(r.getSource().toLowerCase().trim())
        ...
        relationships = relationships.stream()
                .filter(r -> {
                    String key = (r.getSource() + "|" + r.getTarget() + "|" + r.getType()).toLowerCase().trim();
                    return seenRels.add(key);
                })
                .toList();

        return new ExtractionResult(dedupedEntities, relationships);
    }

    private String findSimilarEntity(String normalizedName, List<Entity> existingEntities) {
        for (Entity existing : existingEntities) {
            ...
            if (existingName.equals(normalizedName)) {
                return existing.getName().trim();
            }
            if (isSimilar(normalizedName, existingName)) {
                return existing.getName().trim();
            }
        }
        return null;
    ...
    private boolean isSimilar(String name1, String name2) {
        ...
        if (name1.contains(name2) || name2.contains(name1)) {
            ...
            return maxLen > 0 && (double) diff / maxLen < 0.3;
        }
